...
auch +a+ bleiben, das ist eine Frage der Konvention, und unter +x+ verstehen wir
...
(in **R** hei&szlig;en die Gültigkeitsbereiche "environments", in **S** hießen sie "frames") ist und die 
...
Funktionen kennen Argumente, am besten nicht zu viele (siehe zum Beispiel <<cc>>). 
...
- Obligatorische. Sie sollten keine Voreinstellungen kennen und übergeben üblicherweise Objekte, die von der Funktion verarbeitet werden sollen. Der zu summierende Vektor `x` in `my_sum` ist ein klassisches Beispiel: ein Voreinstellung f&uuml;r die zu summiernden Zahlen (etwa `x = c(40, 2)` ist unsinnig.
- Optionale. Sie sollten Voreinstellungen kennen und dienen üblicherweise der Steuerung der Funktion. Das Argument `order` in `memory_hogs` ist ein klassisches Beispiel: 
...
Bei vielen dieser Funktionen sind die Nebenwirkungen der eigentliche Zweck der Funktion, manchmal ist der Rückgabewert sogar belanglos. 
Die Funktion `rm` gibt beispielsweise unsichtbar `NULL` zurück (und das selbst dann, wenn es gar nichts zu löschen gibt). 
...
Das können wir ändern, indem wir einen expliziten Rückgabewert einbauen (bei der Gelegenheit wandeln wir gleich noch den Gültigkeitsbereich, indem die Funktion l&ouml;scht, in ein Steuerargument mit Voreinstellung um):
...
====  Global 
Funktionen, die Du nicht nur in einem Projekt nutzen willst, 
kannst Du zentral verwalten, 
in dem Du Dateien wie im Abschnitt <<_lokal, lokal>> beschrieben ablegst, 
nur mit dem Unterschied, dass Du ein zentrales Verzeichnis benutzt.
Dann f&uuml;gst Du ineine Deiner **R**-Startup-Dateien die Anweisung
...
Das geht in reinen +base R+, ist aber eher umständlich,
...
----
## RUNIT TEST PROTOCOL -- Tue Apr  3 10:29:53 2018 
## *********************************************** 
...
Aber vorweg stelle ich Dir noch https://CRAN.R-project.org/package=codetools[codetools] vor, es gehört zur Gruppe der "_recommended_ Packages" von **R** und kann Programmfehler finden.
Die drei anderen Pakete gibt es nur, weil https://CRAN.R-project.org/package=codetools[codetools] schon da war.
...
+.golden_ratio+ (der f&uuml;hrende Punkt ist eine Konvention, die aus der UNIX-Welt stammt, 
                 ...
                 aus dem directory listing ausgeschlossen (unter Windows hei&szlig;en diese Dateien "versteckt")
...
und weil dokumentieren Dir so viel Spa&szlig; bereit, dokumentierst Du gleich auch
...
+src/golden_ratio.R+ und +src/.golden_ratio.R+ und ihre zugeh&ouml;rigen Testdateien.

Damit sie den Paketanforderungen gen&uuml;gen, musst Du noch zwei kleine
&Auml;nderungen vornehmen: +golden_ratio.R+ braucht noch einen
...
Und Du solltest immer auch die Deine <<_weniger_schlecht_programmieren,drei Freunde>>  https://CRAN.R-project.org/package=lintr[lintr], https://CRAN.R-project.org/package=cleanr[cleanr] und https://CRAN.R-project.org/package=cyclocomp[cyclocomp]
...
Paket fertig. Die Metadaten sind nat&uuml;rlich gelogen (Du solltest die Datei +DESCRIPTION+ anpassen, Du solltest eine Datei +NEWS.md+ pflegen, Du solltest ein https://de.wikipedia.org/wiki/Versionsverwaltung[version control system] benutzten), aber funktional ist es korrekt.
