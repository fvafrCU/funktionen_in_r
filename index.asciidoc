...
----
## RUNIT TEST PROTOCOL -- Mon Mar 19 11:28:20 2018 
## *********************************************** 
...
[source,r]
----
## Warning in file(filename, "r", encoding = encoding): cannot open file 'src/
## golden_ratio.R': No such file or directory
...
[source,r]
----
## Error in FUN(X[[i]], ...): 'src/golden_ratio.R' is not an existing file
...
Das kannst Du von Hand machen, oder Du schreibst folgende Funktion:


[source,r]
----
rename_funtction <- function(old_name, new_name, directory = ".") {
    file_names <- list.files(directory, full.names = TRUE, 
                             pattern = paste0("^(test_|)", old_name, "\\.[rR]$")
                             )
    new_file_names <- NULL

    for (file_name in file_names) {
        patterns <- c(definition_pattern = paste0(old_name, "( *<-)"),
                      call_pattern = paste0(old_name, "(\\()"))

        new_file_name <- file.path(dirname(file_name), 
                                   sub(old_name, new_name, basename(file_name)))
        x <- readLines(file_name)
        for (pattern in patterns) 
            x <- sub(pattern, paste0(new_name, "\\1"), x)
...
Die &uuml;bernimmt dann die Arbeit:


[source,r]
----
rename_funtction(old_name = "golden_ratio", new_name = ".golden_ratio", 
                 directory = "src")
----
...
[source,r]
----
rename_funtction(old_name = "golden_rectangle", new_name = "golden_ratio", 
                 directory = "src")
----
...
[source,r]
----
document::document(file.path("src", "golden_ratio.R"), output_directory = ".")
----
