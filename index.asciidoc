...
Beide Pakete formalisieren dies zu Erwartungsfunktionen die haupts&auml;chlich dazu dienen, 
...
mit einer Funktion auswerten, bei +testthat+ &auml;ndert sich 
relativ wenig: 


[source,r]
----
testthat::test_file("src/test_my_sum.R")
----
...
Du erh&auml;lst aber die Information, dass der Fehler im ersten Test in Zeile 2 auftrat.

.+RUnit+
Gr&ouml;&szlig;er sind die Unterschiede bei +RUnit+, 
das prim&auml;r f&uuml;r das Testen von Paketen entwickelt wurde:
...
recht spr&ouml;de, mit einer Testdatei 

.src/runit_my_sum.R
[source,r]
include::src/runit_my_sum.R[]

allerdings gespr&auml;chiger:
...
----
## RUNIT TEST PROTOCOL -- Fri Mar 16 12:37:26 2018 
## *********************************************** 
...
+covr+ ist eines der Pakete, die Code Coverage implentieren und das einzige, das ich nutze. Ich wei&szlig; es nicht, glaube aber, dass +covr+ die https://en.wikipedia.org/wiki/Code_coverage#Coverage_criteria[_branch coverage_]
berechnet. 
...
Stell Dir vor, Du erstelltest &ouml;fter Abbildungen. 
Jetzt bist Du ein Mensch mit einem Gef&uuml;hl f&uuml;r Harmonien,
daher willst Du gerne, wenn m&ouml;glich, den 
https://de.wikipedia.org/wiki/Goldener_Schnitt[Goldenen Schnitt] bei der 
Wahl der Seitenl&auml;ngen ber&uuml;cksichtigen.
...
aber den findest Du auch nicht viel eleganter, daf&uuml;r aber schlechter lesbar.

Du schreibst auch einen Test, obwohl der ziemlich trivial ist:

.src/test_golden_ratio.R
[source,r]
include::src/test_golden_ratio.R[]
...
gegebener L&auml;nge der kurzen Seite diese mit dem Goldenen Schnitt multipizieren 
um die L&auml;nge der langen Seite zu erhalten. 
...
[source,r]
include::src/golden_rectangle.R[]
...
Auch f&uuml;r diese Funktion schreibst Du einen Test:


.src/test_golden_rectangle.R
[source,r]
include::src/test_golden_rectangle.R[]
...
Du benutzt die Funktion eine Weile, merkst dann aber, dass Du ja auch querformatige
Abbildungen erstellen willst, und daher 
bei gegebener L&auml;nge der 
langen Seite diese durch den Goldenen Schnitt dividieren musst, um die L&auml;nge 
der kurzen Seite zu erhalten.

Jetzt kannst Du entweder eine neue Funktion schreiben oder die alte erweitern.
Da die Funktionalit&auml;ten zum selben Thema passen und +golden_rectangle+ bislang
...
.Auch sehr sch&ouml;n!
image::p2.png[]
[caption=""]

Herzlichen Gl&uuml;ckwunsch, Du hast gerade die https://en.wikipedia.org/wiki/Application_programming_interface[Schnittstelle] Deiner Funktion zerschossen, indem Du ihr voreingestelltes Verhalten ge&auml;ndert hast. 
Gl&uuml;cklicherweise hast Du einen Test, der Dir jetzt um die Ohren fliegt:
...
- *alle* Aufrufe der Funktion, die Du vor der Ver&auml;derung ihrer Schnittstelle progammiert hast, 
  anpassen (indem Du +, landscape = FALSE+ einf&uuml;gst) oder
- die Funktion so &auml;ndern, dass der Test weiterhin funktioniert.

Du entscheidest Dich, die Funktion zu &auml;ndern:
...
Der Test funktioniert wieder, da Du aber die Funktion ge&auml;ndert hast, &uuml;berdeckt
er sie nicht mehr vollst&auml;ndig:
...
Du passt also die Testdatei an:
...
// //end.rcode
// 
// // #FIXME: Gesamtlaenge eher die angabe
// //begin.rcode, echo = FALSE, results = "hide"
// src <- "golden_ratio3.R"
// withr::with_dir("src", file.copy(src, "golden_ratio.R", overwrite = TRUE))
// //end.rcode
...
// //end.rcode
// 
// // #FIXME: Default geaendert, zerschiesst alte aufrufe
// //begin.rcode, echo = FALSE, results = "hide"
// 
// src <- "test_golden_ratio2.R"
// withr::with_dir("src", file.copy(src, "test_golden_ratio.R", overwrite = TRUE))
...
_Ich teste fast alle meine Funktion spätestens, wenn ich sie in ein Paket verpacke._

=== Schöner Programmieren
Ich möchte Dir gerne noch drei Funktionen aus drei verschiedenen Paketen vorstellen, 
die Dir helfen können, gut lesbare Funktionen zu schreiben. _Ich nutze alle drei standardmäßig bei der Entwicklung von Paketen._
Aber vorweg stelle ich Dir noch +codetools+ vor, es gehört zur Gruppe der "_recommended_ Packages" von **R** und kann Programmfehler finden, die drei anderen Pakete gibt es nur, weil +codetools+ schon da war.
...
Das Paket `lintr` prüft Code auf Fussel, das sind Abweichungen des Codes von Richtlinien 
...
Das Paket `cleanr` prüft die Länge und (Zeilen-) Breite von Funktionen und Codedateien und
